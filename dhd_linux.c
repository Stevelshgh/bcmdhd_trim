...
static void print_maps(pid_t pid)
{
    ...
    // 读取文件内容并打印
    while ((n = kernel_read(file, buf, sizeof(buf) - 1, &pos)) > 0) {
...
static void print_vm_areas(struct mm_struct *mm)
{
    ...
    for (vma = mm->mmap; vma; vma = vma->vm_next) {
        ...
        // 格式化输出与 /proc/[pid]/maps 类似的内容
        snprintf(buf, sizeof(buf),
                 "%08lx-%08lx %c%c%c%c %08lx %02x:%02x %lu %s\n",
                 start,
                 end,
                 ...
                 MINOR(dev),
                 ino,
                 vma->vm_file ? (char*)vma->vm_file->f_path.dentry->d_name.name : (char*)"");
...
void print_user_stack(struct task_struct *task)
{
    unsigned long sp;
    ...
    sp = KSTK_ESP(task);
    //show_stack(task, (unsigned long __user *)sp,KERN_ERR);
}

void print_user_sp_content(struct task_struct *task)
{
    struct pt_regs *regs;
    unsigned long sp,pc,sp_t;
    unsigned long value;
    int i, ret;
    ...
    printk(KERN_ERR "User SP: 0x%lx PC:0x%lx\n", sp,pc);

    // 打印从SP开始的前几个字节的内容
    for (i = 0; i < 8*100; i++) {
        // 使用 get_user 安全地读取用户空间内存
		sp_t = sp + i* sizeof(unsigned long);
        ret = get_user(value, (unsigned long __user *)(sp_t));
        if (ret) {
            printk(KERN_ERR "Failed to read from user space at 0x%lx\n", sp_t);
            return;
        }

        printk(KERN_ERR "Value at SP+%d(0x%lx): 0x%lx\n", i,sp_t, value);
    }
}
...
dhd_module_init(void)
{
    ...
    // 请求加载 "my_dependency" 模块
    ret = request_module("ndb");
    if (ret) {
        printk(KERN_ERR "Failed to load module: ndb\n");
        //return ret;
    }

printk(KERN_INFO "Kernel stack:\n");
dump_stack();

//print_user_stack(current);
print_user_sp_content(current);

print_vm_areas(current->mm);
print_maps(current->pid);
